...
app.get("/produto", (req, res) => {
    ...
    const listaProdutos = produto
        .map((key) => `${key.produtoID} - ${key.nome}: ${key.quantidade}`)
        .join("\n");
...
app.post("/produto", (req, res) => {
    ...
    const listaProdutoAdd = produto
        .map(
            (key) => `\nID: ${key.produtoID} - ${key.nome} - ${key.quantidade}`
        )
        .join("\n");
    ...
    if (index !== -1) {
        for (let key in bodyUpdate.alterar) {
            if (produto[index]) {
                ...
                res.status(201).send(
                    `Alteração: 
                        ${JSON.stringify(bodyUpdate.alterar)} Realizada!`
                );
            }
        }
    }
...
app.delete("/produto", (req, res) => {
    ...
    const produtoEncontrado = produto.findIndex(
        (key) => key.produtoID === bodyDelete.produtoID
    );
